...
ggplot(dat, aes(x=as.numeric(log2.VAF.),)) +geom_density(alpha=.4,aes(colour=Sample),size=1.5)+ylab("kernal density estimate") + xlab("biopsy volume (cm3)")+theme(text = element_text(size=15),axis.text.x = element_text(size=15),axis.text.y = element_text(size=15))+scale_x_continuous()+theme_classic()+scale_color_manual(values = cbPalette)
